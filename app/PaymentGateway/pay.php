    ...
    public function verify($token , $status)
    {
        ...
        if (isset($result->status)) {
            if ($result->status == 1) {
                ...
                \Cart::clear();
                return ['success' =>'تراکنش با موفقیت انجام شد. شماره تراکنش '.$result->transId];
            } else {
                return ['error' => 'تراکنش با خطا مواجه شد. شماره وضعیت '.$result->status ];
            }
        } else {
            if ($status == 0) {
                return ['error' => 'تراکنش با خطا مواجه شد. شماره وضعیت '.$status ];
