...
class SitemapController extends Controller
{
    ...
    public function sitemapProducts()
    {
        ...
        foreach ($products as $product) {
            $images = [];
            array_push($images, ['url' => URL::to(asset(env('PRODUCT_IMAGES_UPLOAD_PATH') . $product->primary_image)), 'title' => $product->name,]);

            foreach ($product->images as $image) {
                array_push($images, ['url' => URL::to(asset(env('PRODUCT_IMAGES_UPLOAD_PATH') . $image->image)), 'title' => $image->name,]);
            }
            $sitemap_products->add($product->slug, $product->updated_at, '1.0', 'monthly', $images);
        }
        // show sitemap
